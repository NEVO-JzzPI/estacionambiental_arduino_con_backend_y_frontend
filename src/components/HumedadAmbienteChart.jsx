...
const HumedadAmbienteChart = () => {
    ...
    axios.get("https://estacionambiental-backend.onrender.com/api/datos")
      .then((res) => {
        const datosTransformados = res.data.map((registro) => ({
          ...
          humedad_ambiente: registro.humedad_ambiente,
        }));
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 bg-white p-6 rounded shadow">
      <h2 className="text-xl font-semibold mb-4 text-center text-gray-700">Humedad Ambiente (%)</h2>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={datosFiltrados}>
          ...
          <YAxis domain={[0, 100]} />
          <Tooltip />
          <Line type="monotone" dataKey="humedad_ambiente" stroke="#06b6d4" strokeWidth={2} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
...
export default HumedadAmbienteChart;
